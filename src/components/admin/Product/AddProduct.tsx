...
const AddProduct: React.FC = () => {
  ...
  const { data: color, refetch } = useGetColorsQuery(); //color
  // alert(color)
  console.log(color);
  ...
  const handleImageRemove = (url: string) => {
    setImg((prevImg: any) =>
      prevImg.filter((imageUrl: string) => imageUrl !== url)
    );
  };
  console.log(sale);
    ...
    <div>
      ...
      >
        ...
        <Form.Item wrapperCol={{ offset: 8, span: 16 }}>
          <Button htmlType="submit">Add New Product</Button>
        </Form.Item>
      </Form>
    </div>
  );
